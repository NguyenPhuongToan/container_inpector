...
def extract_flexitank_number_with_gemini(image_path: Path) -> str:
    if not settings.gemini_configured:
        raise GeminiServiceError("GEMINI_API_KEY is not configured")
    ...
    prompt = (
        ...
        "It is usually printed with hyphens, for example 23TT3-2601-005. "
        "Return only the serial number, preserving hyphens, or NOT_FOUND."
    )
    ...
    text = (getattr(response, "text", "") or "").upper()
    match = FLEXITANK_NUMBER_PATTERN.search(text)
    if not match:
        raise GeminiServiceError("NOT_FOUND")
